...
public class ProductView2 extends JFrame implements ActionListener {
    ...
    public Container createListView(){
        ...
        UserFacade userLoggedFacade = new UserFacade(user);
        List<Product> productsActiveSales = userLoggedFacade.getProductByUser(user);   
       
        List<Product> productsPreviousSales = userLoggedFacade.getProductsByAuthor(user);
       
        System.out.println("SIIIIIIIZE:"+productsActiveSales.size() + "+"+productsPreviousSales.size());
      

        List<Product> sortedProductsActiveSales = new ArrayList<>();
        List<Product> sortedProductsPreviousSales = new ArrayList<>();
        if(!search.getText().equals("")){
            for(Product p : productsActiveSales){
                if(p.getName().contains(search.getText())) sortedProductsActiveSales.add(p);
            }
            for(Product p : productsPreviousSales){
                if(p.getName().contains(search.getText())) sortedProductsPreviousSales.add(p);
            }
        }else{
        	sortedProductsPreviousSales = productsPreviousSales;
        	sortedProductsActiveSales = productsActiveSales;
        }

        Container c = productList.getContentPane();
        Container c1bis = new Container();
        
        c1bis.setLayout(new BorderLayout());
      
      
        Container center1 = new Container();
        c1bis.add(center1, BorderLayout.CENTER);
        center1.setLayout(new WrapLayout());
        for(Product p : sortedProductsActiveSales){

            Container card1 = new Container();
            card1.setLayout(new BorderLayout());
            card1.add(new JLabel(p.getName() + " : " + p.getPrice()), BorderLayout.NORTH);
            JButtonProduct details1 = new JButtonProduct("Voir les détails", p.getId());
            details1.addActionListener(this); 
            card1.add(details1, BorderLayout.SOUTH);
            BufferedImage picture1;
            try {
                picture1 = ImageIO.read(new File("src/main/java/UI/product.png"));
                JLabel picLabel = new JLabel(new ImageIcon(picture1));
                card1.add(picLabel, BorderLayout.CENTER);
                center1.add(card1);
            } catch (Exception e) {
                e.printStackTrace(); 
            } 
        ...
        for(Product p : sortedProductsPreviousSales){
    ...
    public void activesSales(Boolean bool) {
    	js1.setVisible(bool);
    }
    public void previousSales(Boolean bool) {
    	js.setVisible(bool);
    }
